...
weather_df %>% 
  ...
  labs(
    ...
    x = "Minimum daily temeperature (C)",
    y = "Maximum daily temeperature (C)",
...
weather_df %>% 
  ...
  viridis::scale_color_viridis(
    name = "Location", 
    discrete = TRUE) +
  theme_minimal() +  ##classic, bw  or use ggthemes::theme_excel()
  theme(legend.position = "bottom") ##放最后不然会被 rewrite
```
...
fas_data %>% 
  ...
  drop_na() %>% 
  mutate(outcome = forcats::fct_relevel(outcome,"pd_ears","pd_pivot","pd_eyes")) %>% 
